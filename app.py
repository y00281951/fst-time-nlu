...
from src.chinese.fst_time_extractor import FstTimeExtractor as ChineseExtractor
from src.english.fst_time_extractor import FstTimeExtractor as EnglishExtractor


# 页面配置
st.set_page_config(
    page_title="FST Time NLU",
    page_icon="⏰",
    layout="wide",
...
def main():
    ...
    st.markdown('<div class="sub-header">基于有限状态转换器的时间表达式识别与解析</div>', unsafe_allow_html=True)
    ...
    with col_left:
        ...
        with col_config4:
            ...
            selected_example = st.selectbox("💡 示例", [""] + examples, format_func=lambda x: "选择示例..." if x == "" else x, key="example_selector")
        ...
        query_text = st.text_area(
            "📝 输入文本",
            height=120,
            placeholder="请输入包含时间表达式的文本..." if language == "中文" else "Enter text with time expressions...",
            key="query_input"
        )
        ...
        if extract_button and query_text:
            with st.spinner("⏳ 识别中..."):
                try:
                    ...
                    datetime_results, query_tag = extractor.extract(query_text, base_time=base_time_str)
